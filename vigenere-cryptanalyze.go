...
func showUsage() {
	...
	fmt.Print("usage: vigenere-cryptanalyze <ciphertext-filename>\n")
	fmt.Print("usage: vigenere-cryptanalyze <ciphertext-filename> <keylength>\n\n")
}
...
func guessKeyLength(cipherText string) int {
	...
	keyLen := hcf(hcf(index[0],index[1]),hcf(index[2],index[3]))
	if keyLen == 1 {
		sort.Ints(index)
		keyLen = index[0] + 1
	}
